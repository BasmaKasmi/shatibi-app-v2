...
interface CSeanceReadOnlyProps {
  abstract: string;
  homework: string;
  abUrlList: Document[];
  hwUrlList: Document[];
...
const CSeanceReadOnly = ({
  ...
  date,
  firstName,
}: CSeanceReadOnlyProps) => {
  const [activeTab, setActiveTab] = useState<
    "Résumé du cours" | "Devoir collectif"
  >("Résumé du cours");
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-md mx-auto py-1 overflow-hidden">
      <h2 className="text-lg font-bold text-center">
        Cahier de séance {firstName ? `de ${firstName}` : ""}
      </h2>
...
export default CSeanceReadOnly;
