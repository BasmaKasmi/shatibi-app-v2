...
export const StudentStatistics = ({
  ...
  const fetchStudentsData = async () => {
    ...
    try {
      ...
      console.log("Réponse complète de getStudents:", response);
      console.log("Résultat des étudiants:", response.result);

      if (response.result && studentId) {
        const currentStudent = response.result.find(
          (s: StudentData) => s.id === studentId
        );
        if (currentStudent) {
          console.log("Données de l'étudiant actuel:", currentStudent);
        }
      }
  ...
  useEffect(() => {
    ...
    loadExistingData();
  }, [groupId, teacherId, studentId, date]);
