...
export interface CorrespondenceV2Props {
  name: string;
  groupId: number;
  teacherId: number;
  studentId: number;
  date: string;
  onComplete?: () => void;
  defaultTab?: "liaison" | "devoir";
  correspondenceMode?: "liaison" | "homework" | null;
  onContentUpdated?: () => void;
  onImmediateUpdate?: (liaison?: string, homework?: string) => void;
}
...
export const CorrespondenceV2 = ({
  name,
  studentId,
  date,
  onComplete,
  defaultTab = "liaison",
  correspondenceMode: propsCorrespondenceMode,
  onContentUpdated,
}: CorrespondenceV2Props) => {
  ...
  const urlCorrespondenceMode = search.get("correspondenceMode") as
    | "liaison"
    | "homework"
    | null;

  const correspondenceMode = propsCorrespondenceMode || urlCorrespondenceMode;

  console.log(
    "correspondenceMode récupéré dans CorrespondenceV2:",
    correspondenceMode
  );
  console.log("URL complète:", window.location.href);
  console.log("Tous les paramètres:", Object.fromEntries(search.entries()));
  ...
  return (
    <div className="flex flex-col items-center justify-start w-full max-w-md mx-auto py-0 overflow-hidden">
      ...
      <div>
        ...
        {activeTab === "Carnet de liaison" ? (
          <Button
            className="w-full py-3 mt-3 text-shatibi-green bg-shatibi-green/[.15] rounded-full font-bold"
...
export default CorrespondenceV2;
